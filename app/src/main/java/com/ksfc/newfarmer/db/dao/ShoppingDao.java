...
public class ShoppingDao extends BaseDao {
    ...
    public synchronized boolean updateShoppingAdditions(String SkUId, String additions) {
        if (isDBOpen()) {
            ContentValues values = new ContentValues();
            values.put(COLUMN_ADDITIONS, additions);
            if (database.update(TABLE_NAME_SHOPPING, values, COLUMN_SKUId
                    + " = ?", new String[]{SkUId}) > 0) {
                RndLog.i(TAG, "呵呵，我将附加项目更新为：" + additions);
                return true;
            }
        }
        return false;
